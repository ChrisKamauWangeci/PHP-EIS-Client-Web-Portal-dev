<x-admin-layout title="">
    ...
    <form method="get"
          ...
          id="searchform"
          action="{{ route('admin.platform-configurations.index') }}">
        <div class="row">
            <div class="col-6 col-md-4 col-lg-3 col-xl-2 pt-2">
                <x-form.select name="company"
                               label="Company"
                               :options="$companies"
                               empty="-"
                               :default="request('company')" />
            </div>
            <div class="col-6 col-md-4 col-lg-3 col-xl-2 pt-2">
                <x-form.input name="platform"
                              id="platform"
                              label="Platform"
                              :value="request('platform')" />
            </div>
            <div class="col-6 col-md-4 col-lg-3 col-xl-2 pt-2">
                <x-form.input name="order_type"
                              id="order_type"
                              label="Order Type"
        ...
        <table class="table table-sm table-hover1 table-bordered w-auto">
            <thead>
                <tr>
                    ...
                    <th><a
                           href="{{ Request::fullUrlWithQuery(['sort_field' => 'platform', 'sort_direction' => $sort_direction]) }}">Platform</a>
                    </th>
                    <th><a
                           href="{{ Request::fullUrlWithQuery(['sort_field' => 'order_type', 'sort_direction' => $sort_direction]) }}">Order
                            Type</a></th>
                    <th><a
                           href="{{ Request::fullUrlWithQuery(['sort_field' => 'submission_type', 'sort_direction' => $sort_direction]) }}">Submission
                            Type</a></th>
                    <th><a
                           href="{{ Request::fullUrlWithQuery(['sort_field' => 'wait_days', 'sort_direction' => $sort_direction]) }}">Wait
                            Days</a></th>
                    <th><a
                           href="{{ Request::fullUrlWithQuery(['sort_field' => 'sequence', 'sort_direction' => $sort_direction]) }}">Sequence</a>
                    </th>
                    <th><a
            ...
            <tbody>
                @foreach ($platformConfigurations as $platformConfiguration)
                    <tr>
                        ...
                        <td><img src="/img/icon_{{ $platformConfiguration->is_active }}.png"
                                 alt=""></td>
    ...
    <a href="{{ route('admin.platform-configurations.create') }}"
       class="btn btn-sm btn-secondary">Add</a>

    <br />
    <br />

    <div hx-get="{{ route('admin.platform-configurations.create') }}"
         hx-swap="outerHTML"
         class="btn btn-sm btn-secondary">Add (HTMX)</div>
