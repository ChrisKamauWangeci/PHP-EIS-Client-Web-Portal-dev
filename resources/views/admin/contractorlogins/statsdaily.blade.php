<x-admin-layout title="">
    ...
    <form method="get"
          ...
          id="searchform"
          action="{{ route('admin.contractorlogins.statsdaily') }}">
        ...
        <div class="row">
            ...
            <div class="col-6 col-md-4 col-lg-3 col-xl-2 pt-2">
                <br />
                <x-form.checkbox name="csv_stats"
                                 id="csv_stats"
                                 label="Export Stats"
                                 :checked="request('csv_stats')" />
            </div>

            <div class="col-6 col-md-4 col-lg-3 col-xl-2 pt-2">
                <br />
                <x-form.checkbox name="csv_summary"
                                 id="csv_summary"
                                 label="Export Summary"
                                 :checked="request('csv_summary')" />
            </div>

            <div class="col-6 col-md-4 col-lg-3 col-xl-2 pt-2">
                <br />
                <x-form.button>Submit</x-form.button>
                <a href="{{ route('admin.contractorlogins.statsdaily') }}"
        ...
        <table class="tablesort table table-hover table-bordered table-sm table-hover w-auto">
            ...
            <tbody>
                @foreach ($contractorlogins as $contractorlogin)
                    <tr>
                        ...
                        <td><a
                               href="{{ route('admin.contractorlogins.index', ['contractor' => $contractorlogin->contractor, 'from' => $contractorlogin->login_date, 'to' => $contractorlogin->login_date]) }}">{{ $contractorlogin->login_count }}</a>
                        </td>
                        <td>{{ $contractorlogin->ip_addresses }}</td>
                        <td><a
                               href="{{ route('admin.filetransfers.index', ['type' => 'upload', 'contractor' => $contractorlogin->contractor, 'from' => $contractorlogin->login_date, 'to' => $contractorlogin->login_date]) }}">{{ $contractorlogin->total_uploads }}</a>
                        </td>
                        <td><a
                               href="{{ route('admin.filetransfers.index', ['type' => 'download', 'contractor' => $contractorlogin->contractor, 'from' => $contractorlogin->login_date, 'to' => $contractorlogin->login_date]) }}">{{ $contractorlogin->total_downloads }}</a>
                        </td>
                        <td>{{ $contractorlogin->total_page_views }}</td>
                        <td>{{ $contractorlogin->total_time_on_site }}</td>
                        <td class="{{ $contractorlogin->total_time_on_site > 8 * 3600 ? 'text-primary' : '' }}">
                            {{ $contractorlogin->formatted_time }}</td>
                        <td class="{{ $contractorlogin->regular_seconds >= 8 * 3600 ? 'text-success' : '' }}">
                            {{ $contractorlogin->regular_time }}</td>
                        <td class="{{ $contractorlogin->overtime_seconds > 0 ? 'text-danger' : '' }}">
                            {{ $contractorlogin->overtime_time ?? 0 }}</td>
                        <td>
            ...
            <tfoot class="fw-bold">
                <tr>
                    ...
                    <td>{{ $totals['time'] }}</td>
                    <td>{{ sprintf('%02d%s', floor($totals['time'] / 3600), gmdate(':i:s', $totals['time'] % 3600)) }}
                    </td>
                    <td>{{ sprintf('%02d%s', floor($totals['regular_time'] / 3600), gmdate(':i:s', $totals['regular_time'] % 3600)) }}
                    </td>
                    <td class="text-danger">
                        {{ sprintf('%02d%s', floor($totals['over_time'] / 3600), gmdate(':i:s', $totals['over_time'] % 3600)) }}
                    </td>
                    <td></td>
                    <td></td>
                    <td></td>
        ...
        <table class="table table-hover table-bordered table-sm w-auto tablesort">
            ...
            <tfoot>
                <tr class="fw-bold">
                    ...
                    <td>{{ sprintf('%02d', floor($totals['time'] / 3600)) . gmdate(':i:s', $totals['time'] % 3600) }}
                    </td>
                    <td class="text-success">
