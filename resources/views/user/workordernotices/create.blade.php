<x-user-layout title="">
    ...
    <div class="row">
        <div class="col-auto">
            <h1>Workorder {{ $days }} Day Notice: {{ $workorder->W_WorkOrder }} - {{ $workorder->W_FirstName }} {{ $workorder->W_LastName }}</h1>
        </div>
    ...
    <div class="row">
        <div class="col-12 col-sm-6">
            ...
            <form method="post" action="{{ route('user.workordernotices.store') }}">
                @csrf

                <input type="hidden" name="workorder_id" value="{{ $workorder->W_WorkOrder }}">
                <input type="hidden" name="workordernotice_id" value="{{ $workordernotice->id }}">
                <input type="hidden" name="days" value="{{ $days }}">

                <h5>Send Email</h5>

                <x-form.checkbox name="sendemail" label="Send Email ?" id="sendemail" checked onchange="hidediv();" />
                <br />
                ...
                <div id="emailblock">
                    ...
                    <x-form.input type="text" name="recipient" id="recipient" label="Recipient" :value="old('recipient', $recipient)" required />
                    <br />

                    <x-form.input type="subject" name="subject" id="subject" label="Subject" :value="old('subject', $subject)" maxlength="80" required />
                    <br />

                    <label for="body">Message</label>
                    <br />
                    <span class="py-1 text-black-50">{!! nl2br($bodyheader) !!}</span>
                    <x-form.textarea name="body" id="body" value="{!! old('body', $body) !!}" rows="8" maxlength="800" />
                    <span class="py-1 text-black-50">{!! nl2br($bodyfooter) !!}</span>
                    <input type="hidden" name="bodyheader" value="{!! $bodyheader !!}">
                    <input type="hidden" name="bodyfooter" value="{!! $bodyfooter !!}">
                    <br />
                    <br />
                ...
                <x-form.select name="W_Owner" :options="$contractors" :default="old('W_Owner', $workordernotice->user_before ?? '')" empty="-" required />

                <br />

                Change Follow up Date
                <x-form.input type="date" name="W_FollowUpDt" :value="old('W_FollowUpDt', $workorder->W_FollowUpDt?->format('Y-m-d'))" />
                <br />

                @if (is_file('//ftpserver/documents/sign/' . $workorder->W_WorkOrder . '-prefill-gs.pdf'))
                    Special Authorization: <a href="/user/files?file=//ftpserver/documents/sign/{{ $workorder->W_WorkOrder }}-prefill-gs.pdf&amp;download=0" target="_blank">{{ $workorder->W_WorkOrder }}-prefill-gs.pdf</a>
                    <br />
                    <input type="hidden" name="file" value="//ftpserver/documents/sign/{{ $workorder->W_WorkOrder }}-prefill-gs.pdf">
                    <x-form.checkbox name="send_attachment" label="Send email with Special Authorization ?" checked />
                    <br />
                @else
        ...
        <div class="col-12 col-sm-6">
            ...
            <div class="overflow-auto bg-white p-2 border expandables" style="height: 300px;  word-break: break-all;" id="statusnotetext">
            ...
            <div class="overflow-auto bg-white p-2 border expandables" style="height: 300px; word-break: break-all;" id="followupstatustext">
