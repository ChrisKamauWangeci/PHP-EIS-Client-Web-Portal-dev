<x-user-layout title="">
    ...
    <form method="POST"
          action="{{ route('user.timecards.clockin') }}">
        @csrf
    ...
    <form method="POST"
          action="{{ route('user.timecards.breakstart') }}">
        @csrf
    ...
    <form method="POST"
          action="{{ route('user.timecards.breakend') }}">
        @csrf
    ...
    <form method="POST"
          action="{{ route('user.timecards.clockout') }}">
        @csrf
    ...
    <table class="table mt-3">
        ...
        @foreach ($timecards as $card)
            <tr>
                ...
                <td>
                    <a href="{{ route('user.timecards.show', $card) }}"
                       class="btn btn-sm btn-warning">Show</a>
                    <form method="POST"
                          action="{{ route('user.timecards.destroy', $card) }}"
