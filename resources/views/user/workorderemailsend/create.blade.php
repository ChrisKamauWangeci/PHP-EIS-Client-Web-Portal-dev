<x-user-layout title="">
    ...
    <div class="row">
        <div class="col-12 col-sm-10 col-md-9 col-lg-8">
            ...
            <form method="post" action="{{ route('user.workorderemailsend.store') }}">
                @csrf

                <input type="hidden" name="workorder_id" value="{{ $workorder->W_WorkOrder }}">
                <input type="hidden" name="type" value="{{ $type }}">
                <input type="hidden" name="niceType" value="{{ $niceType }}">

                <x-form.input type="email" name="sender" id="sender" label="Sender" :value="old('sender', $sender)" readonly required />
                <br />

                <x-form.input type="email" name="recipient" id="recipient" label="Recipient" :value="old('recipient', $recipient)" required />
                <br />

                <x-form.input type="text" name="subject" id="subject" label="Subject" :value="old('subject', $subject)" maxlength="80" required />
                <br />

                <x-form.textarea name="body" id="body" :value="old('body', $body)" rows="30" maxlength="5000" />
                <br />
                ...
                @if (is_file($companylor))
                    <input type="hidden" name="attachment_1" value="{{ $companylor }}" />
                    <x-form.checkbox name="attachment_1_checkbox" label="Company LOR - {{ $companylor }} {{ round(filesize($companylor) / 1024 / 1024, 2) }} MB" />
                @endif

                @if (is_file($insurancecompanylor))
                    <input type="hidden" name="attachment_2" value="{{ $insurancecompanylor }}" />
                    <x-form.checkbox name="attachment_2_checkbox" label="Insurance Company LOR - {{ $insurancecompanylor }} {{ round(filesize($insurancecompanylor) / 1024 / 1024, 2) }} MB" />
                @endif

                @if (is_file($filepdf))
                    <input type="hidden" name="attachment_3" value="{{ $filepdf }}" />
                    <x-form.checkbox name="attachment_3_checkbox" label="Authorization File PDF - {{ $filepdf }} {{ round(filesize($filepdf) / 1024 / 1024, 2) }} MB" />
                @endif

                @if (is_file($filetif))
                    <input type="hidden" name="attachment_4" value="{{ $filetif }}" />
                    <x-form.checkbox name="attachment_4_checkbox" label="Authorization File TIF - {{ $filetif }} {{ round(filesize($filetif) / 1024 / 1024, 2) }} MB" />
                @endif

                @foreach ($files as $file)
                    <input type="hidden" name="attachment_{{ 5 + $loop->index }}" value="{{ $file }}" />
                    <x-form.checkbox name="attachment_{{ 5 + $loop->index }}_checkbox" label="Request File - {{ $file }} {{ round($file->getSize() / 1024 / 1024, 2) }} MB" />
                @endforeach
