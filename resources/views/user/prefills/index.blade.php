<x-user-layout title="">
    ...
    <form method="get" accept-charset="utf-8" id="searchform" action="{{ route('user.prefills.index') }}">

        <div class="row">

            <div class="col-6 col-md-2 pt-2">
                <x-form.input name="workorder_id" label="Workorder ID" :value="request('workorder_id')" autocomplete="off" maxlength="50" />
            </div>

            <div class="col-6 col-md-2 pt-2">
                <x-form.input name="slug" label="Slug" :value="request('slug')" autocomplete="off" maxlength="50" />
            </div>

            <div class="col-6 col-md-2 pt-2">
                <x-form.input name="applicant" label="Applicant" :value="request('applicant')" autocomplete="off" maxlength="50" />
            </div>

            <div class="col-6 col-md-2 pt-2">
                <x-form.input name="username" label="Username" :value="request('username')" autocomplete="off" maxlength="50" />
            </div>
            ...
            <div class="col-6 col-md-2 pt-2">
                ...
                <a href="{{ route('user.prefills.index') }}" class="btn btn-sm btn-secondary">Reset</a>
            </div>
        ...
        <table class="table table-sm table-hover table-bordered w-auto">
            <thead>
                <tr>
                    ...
                    <th><a href="{{ Request::fullUrlWithQuery(['sort_field' => 'db', 'sort_direction' => $sort_direction]) }}">DB</a></th>
                    <th><a href="{{ Request::fullUrlWithQuery(['sort_field' => 'workorder_id', 'sort_direction' => $sort_direction]) }}">Workorder ID</a></th>
                    <th><a href="{{ Request::fullUrlWithQuery(['sort_field' => 'applicant', 'sort_direction' => $sort_direction]) }}">Applicant</a></th>
                ...
                @foreach ($prefills as $prefill)
                    <tr>
                        ...
                        <td nowrap>
                            ...
                            @endphp
                            @if (is_file($file))
                                <a href="/user/files?file=//ftpserver/documents/sign/{{ $prefill->workorder_id }}-prefill-gs.pdf&amp;download=0" target="_blank" class="btn btn-xs btn-secondary">View GS</a>
                                <a href="/user/files?file=//ftpserver/documents/sign/{{ $prefill->workorder_id }}-prefill-gs.pdf&amp;download=1" target="_blank" class="btn btn-xs btn-secondary">Download GS</a>
                            @endif
                        </td>
                        {{-- <td nowrap>
                            @php
    ...
    <a href="{{ route('user.prefills.stats') }}" class="btn btn-sm btn-secondary">Stats</a>
