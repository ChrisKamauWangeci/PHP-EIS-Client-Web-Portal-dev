<x-admin-layout title="">
    ...
    <form method="get" accept-charset="utf-8" id="searchform" action="{{ route('admin.contractorlogins.index') }}">
        ...
        <div class="row">
            ...
            <div class="col-6 col-md-4 col-lg-3 col-xl-2 pt-2">
                <x-form.input type="date" name="from" label="From" :value="request('from')" min="{{ now()->subYear(5)->format('Y-m-d') }}" max="{{ now()->addDays(1)->format('Y-m-d') }}" />
            </div>

            <div class="col-6 col-md-4 col-lg-3 col-xl-2 pt-2">
                <x-form.input type="date" name="to" label="To" :value="request('to')" min="{{ now()->subYear(5)->format('Y-m-d') }}" max="{{ now()->addDays(1)->format('Y-m-d') }}" />
            </div>

            <div class="col-6 col-md-4 col-lg-3 col-xl-2 pt-2">
                <br />
                <x-form.button>Submit</x-form.button>
                <a href="{{ route('admin.contractorlogins.index') }}" class="btn btn-sm btn-secondary">Reset</a>
            </div>
        ...
        <table class="table table-hover table-bordered table-sm table-hover w-auto">
            <thead>
                <tr>
                    ...
                    <th><a href="{{ Request::fullUrlWithQuery(['sort_field' => 'page_views', 'sort_direction' => $sort_direction]) }}">Page Views</a></th>
                    <th>Uploads</th>
                    <th>Downloads</th>
                    <th>
                        <a href="{{ Request::fullUrlWithQuery(['sort_field' => 'created_at', 'sort_direction' => $sort_direction]) }}">Created At</a>
                        <a href="{{ Request::fullUrlWithQuery(['sort_field' => 'updated_at', 'sort_direction' => $sort_direction]) }}">Updated At</a>
                    </th>
                    <th><a href="{{ Request::fullUrlWithQuery(['sort_field' => 'logout_at', 'sort_direction' => $sort_direction]) }}">Logout At</a></th>
                </tr>
            </thead>
            <tbody>
