<x-user-layout title="">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            var elapsed = function() {
                var from = new Date(new Date().setDate(new Date().getDate() - this.getAttribute(
                    "data-elapsed")));
    ...
    Received date:
    <span class="elapseddays text-decoration-underline"
          data-elapsed="30">last 30 days</span>
    &nbsp;
    <span class="elapseddays text-decoration-underline"
          data-elapsed="60">last 60 days</span>
    &nbsp;
    <span class="elapseddays text-decoration-underline"
          data-elapsed="90">last 90 days</span>
    &nbsp;
    <span class="elapseddays text-decoration-underline"
          data-elapsed="120">last 120 days</span>
    &nbsp;
    <span class="elapseddays text-decoration-underline"
          data-elapsed="150">last 150 days</span>

    <form method="post"
          accept-charset="utf-8"
          id="searchform"
          action="{{ route('user.workorders.prg') }}">
        @csrf
        ...
        <div class="row">
            ...
            <div class="col-4 col-md-4 col-lg-2 pt-2">
                <x-form.select name="W_Owner"
                               label="Assigned To"
                               :options="$contractorsselectswithempty"
                               empty="-"
            ...
            <div class="col-4 col-md-4 col-lg-2 pt-2">
                <x-form.select name="W_InsCompany"
                               label="Agency"
                               :options="$agencies"
                               empty="-"
                               :default="request('W_InsCompany')" />
            </div>
            ...
            <div class="col-6 col-md-3 col-lg-2 pt-2">
                <x-form.input name="completedfrom"
                              id="completedfrom"
            ...
            <div class="col-4 col-md-4 col-lg-2 pt-2">
                Workorder Hold<br />
                <x-form.checkbox name="is_hold"
                                 id="is_hold"
                                 label="On Hold ?"
                                 checked="{{ request('is_hold') }}" />
            </div>
            ...
            <div class="col-4 col-md-4 col-lg-2 pt-2">
                ...
                @endphp
                <x-form.select name="hold_reason"
                               label="Hold Reason"
                               :options="$options"
                               empty="-"
                               :default="request('hold_reason')" />
            </div>
            ...
            <div class="col-4 col-md-4 col-lg-2 pt-2">
                ...
                <a href="{{ route('user.workorders.index') }}"
            ...
            @if (!empty($workorders) && $usersession['contractor']['accesslevel'] && $usersession['contractor']['C_SysAdmin'])
                <div class="col-4 col-md-4 col-lg-2 pt-2">
                    ...
                    <br />
                    <a
                       href="{{ route('user.workorders.export', array_merge(request()->query(), ['exporttype' => 'csv'])) }}">Export
                        CSV</a>
                    &nbsp;
                    <a
                       href="{{ route('user.workorders.export', array_merge(request()->query(), ['exporttype' => 'xlsx'])) }}">Export
                        Excel</a>
                </div>
            @endif
    ...
    <form method="post"
          action="{{ route('user.workorders.transfer') }}">
                ...
                <table class="table table-sm table-hove1r table-striped table-bordered w-auto">
                    <thead>
                        <tr>
                            <th>
                                <a
                                   href="{{ Request::fullUrlWithQuery(['sort_field' => 'W_WorkOrder', 'sort_direction' => $sort_direction]) }}">WO</a>
                                <input type="checkbox"
                                       onclick="toggle(this);" />
                            </th>
                            <th><a
                                   href="{{ Request::fullUrlWithQuery(['sort_field' => 'age', 'sort_direction' => $sort_direction]) }}">Age</a>
                            </th>
                            <th>
                                <a
                                   href="{{ Request::fullUrlWithQuery(['sort_field' => 'W_FirstName', 'sort_direction' => $sort_direction]) }}">First</a>
                                <a
                                   href="{{ Request::fullUrlWithQuery(['sort_field' => 'W_LastName', 'sort_direction' => $sort_direction]) }}">Last</a>
                                Name
                            </th>
                            <th><a
                                   href="{{ Request::fullUrlWithQuery(['sort_field' => 'W_Hospital', 'sort_direction' => $sort_direction]) }}">Hospital</a>
                            </th>
                            <th><a
                                   href="{{ Request::fullUrlWithQuery(['sort_field' => 'Hospital_timezone_offset', 'sort_direction' => $sort_direction]) }}">T</a>
                            </th>
                            <th><a
                                   href="{{ Request::fullUrlWithQuery(['sort_field' => 'W_Contractor', 'sort_direction' => $sort_direction]) }}">Case
                                    Manager</a></th>
                            <th><a
                                   href="{{ Request::fullUrlWithQuery(['sort_field' => 'W_Owner', 'sort_direction' => $sort_direction]) }}">Assigned
                                    To</a></th>
                            <th><a
                                   href="{{ Request::fullUrlWithQuery(['sort_field' => 'Requestor_R_Company', 'sort_direction' => $sort_direction]) }}">Company</a>
                            </th>
                            @if (request('is_hold') == 1)
                                <th><a
                                       href="{{ Request::fullUrlWithQuery(['sort_field' => 'workorderholdtimes.reason', 'sort_direction' => $sort_direction]) }}">Hold
                                        Reason</a></th>
                                <th><a
                                       href="{{ Request::fullUrlWithQuery(['sort_field' => 'workorderholdtimes.date_start', 'sort_direction' => $sort_direction]) }}">Hold
                                        Start</a></th>
                                <th><a
                                       href="{{ Request::fullUrlWithQuery(['sort_field' => 'workorderholdtimes.date_start', 'sort_direction' => $sort_direction]) }}">Age</a>
                                </th>
                            @endif
                            <th><a
                                   href="{{ Request::fullUrlWithQuery(['sort_field' => 'W_FollowUpStatus', 'sort_direction' => $sort_direction]) }}">Follow
                                    up Status</a></th>
                            <th>P</th>
                            <th><a
                                   href="{{ Request::fullUrlWithQuery(['sort_field' => 'W_FollowUpDt', 'sort_direction' => $sort_direction]) }}">Follow
                                    up Date</a></th>
                            <th><a
                                   href="{{ Request::fullUrlWithQuery(['sort_field' => 'W_UpdDate', 'sort_direction' => $sort_direction]) }}">Last
                                    Update</a></th>
                        </tr>
                    </thead>
                    <tbody>
                            ...
                            <tr>
                                <td>
                                    @if (!$database)
                                        ...
                                        @endif
                                    @else
                                        <a href="https://{{ $database }}.expressimagingservices.net/user/workorders/{{ $workorder->W_WorkOrder }}"
                                           target="_blank">{{ $workorder->W_WorkOrder }}</a>
                                    @endif
                                ...
                                <td>{{ $workorder->W_FirstName }} {{ $workorder->W_MiddleInit }}
                                    {{ $workorder->W_LastName }}</td>
                                <td>
                                    @if ($workorder->W_Hospital)
                                        <span data-bs-toggle="tooltip"
                                              data-bs-placement="right"
                                              data-bs-html="true"
                                              data-bs-title="{{ $workorder->W_Hospital }}<br />{{ $workorder->Hospital_H_Hospital2 }}<br />{{ $workorder->Hospital_H_Phone }}<br />{{ $workorder->Hospital_H_City }} {{ $workorder->Hospital_H_State }}, {{ $workorder->Hospital_H_Zip }}">
                                ...
                                @if (request('is_hold') == 1)
                                    ...
                                    <td>{{ (int) $workorder->hold_date_start?->diffInDays($workorder->hold_date_end) ?? 'N/A' }}
                                    </td>
                                @endif
                                <td class="small">
                                    @php
                                ...
                                <td nowrap
                                    class="small">{{ $workorder->W_ImagePages }}</td>
                                <td nowrap
                                    class="small">{{ $workorder->W_FollowUpDt?->format('m/d/Y') }}</td>
                                <td nowrap
                                    class="small">{{ $workorder->W_UpdDate?->format('m/d/Y g:i a') }}
                                    {{ $workorder->W_UpdDate ? 'pst' : '' }}</td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
