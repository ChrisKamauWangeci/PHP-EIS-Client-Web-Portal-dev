<tr data-id="{{ $apscancellation->CancellationID }}"
    id="item-{{ $apscancellation->CancellationID }}"
    @class(['flash' => $flash ?? false])>
    <td>
        <a
           href="{{ route('user.workorders.show', $apscancellation->EISWorkOrderID) }}">{{ $apscancellation->EISWorkOrderID }}</a>
    </td>
    ...
    <td>
        ...
        <form method="post">
            ...
            <input type="hidden"
                   name="CancellationID"
                   value="{{ $apscancellation->CancellationID }}">

            <x-form.select hx-post="{{ route('user.apscancellations.update', $apscancellation->CancellationID) }}"
    ...
    <td class="align-middle">
        ...
        @else
            <form method="post">
                ...
                <input hx-post="{{ route('user.apscancellations.update', $apscancellation->CancellationID) }}"
                       hx-target="#item-{{ $apscancellation->CancellationID }}"
                       hx-swap="outerHTML"
                       hx-trigger="change"
                       hx-include="closest form"
    ...
    <td class="actions">
        <a href="{{ route('user.apscancellations.show', $apscancellation->CancellationID) }}"
        ...
        <i id="loading-{{ $apscancellation->CancellationID }}"
           class="fa-solid fa-spinner fa-spin htmx-indicator"></i>
    </td>
</tr>
