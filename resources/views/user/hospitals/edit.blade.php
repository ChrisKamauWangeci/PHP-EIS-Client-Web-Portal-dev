<x-user-layout title="">
        ...
        <form method="post" action="{{ route('user.hospitals.update', $hospital->H_ID) }}" id="hospitalform">
            ...
            <div class="row">
                <div class="col-6">
                    ...
                    <x-hospitalformcopyschedule :hospital=$hospital />
            ...
            <x-hospitalformnotes :hospital=$hospital />
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function() {
            // Prevent keydown on elements with class 'readonly'
            ...
            document.getElementById("alternatepaymentclear").addEventListener("click", function() {
                document.getElementById("alternatepayment").value = '';
            });

            document.getElementById("copyserviceclear").addEventListener("click", function() {
                document.getElementById("copyservice").value = '';
            });
        ...
        function popup(url) {
            window.open(url, "popuphospital", "scrollbars=yes,width=1280,height=800,resizable=yes,left=40,top=40")
        }
    </script>
    ...
    <script>
        let app = Vue.createApp({
            ...
            methods: {
                ...
                getCopyservice() {
                    this.infowindowspin();
                    var C_CopyService = document.getElementById("copyservice").value;

                    const res = fetch('/api/copyservices/show?C_CopyService=' + encodeURIComponent(C_CopyService))
                        .then(response => response.json())
                        .then(data => {
                            if (data) {
                                console.log(data);
                ...
                getRoi() {
                    this.infowindowspin();
                    var R_ROIname = document.getElementById("roi").value;

                    const res = fetch('/api/rois/show?R_ROIname=' + encodeURIComponent(R_ROIname))
                        .then(response => response.json())
                        .then(data => {
                            if (data) {
                                console.log(data);
