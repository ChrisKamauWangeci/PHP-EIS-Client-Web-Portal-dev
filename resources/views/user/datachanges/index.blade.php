<x-user-layout title="">
    ...
    <form method="get"
          ...
          id="searchform"
          action="{{ route('user.datachanges.index') }}">
        <input type="hidden"
               name="search"
               value="1">
        <input type="hidden"
        ...
        <div class="row">
            ...
            <div class="col-4 col-md-2 col-lg-2 pt-2">
                <x-form.input name="foreign_key"
                              label="Foreign Key"
                              :value="request('foreign_key')"
                              maxlength="50" />
            </div>

            <div class="col-4 col-md-2 col-lg-2 pt-2">
                <x-form.input name="data"
                              label="Data"
                              :value="request('data')"
                              maxlength="50" />
            </div>
        ...
        <table class="table table-sm table-hover table-bordered w-auto">
            <thead>
                <tr>
                    ...
                    <th><a
                           href="{{ Request::fullUrlWithQuery(['sort_field' => 'model', 'sort_direction' => $sort_direction]) }}">Model</a>
                    </th>
                    <th><a
                           href="{{ Request::fullUrlWithQuery(['sort_field' => 'foreign_key', 'sort_direction' => $sort_direction]) }}">Foreign
                            Key</a></th>
                    <th><a
                           href="{{ Request::fullUrlWithQuery(['sort_field' => 'data', 'sort_direction' => $sort_direction]) }}">Data</a>
                    </th>
                    <th><a
            ...
            <tbody>
                @foreach ($datachanges as $datachange)
                    <tr>
                        ...
                        <td><a
                               href="/user/{{ $datachange->model }}/{{ $datachange->foreign_key }}">{{ $datachange->foreign_key }}</a>
                        </td>
                        <td>{!! nl2br($datachange->data ?? '') !!}</td>
                        <td>{{ $datachange->created_by }}</td>
                        <td>{{ $datachange->created_at }}</td>
                        <td class="actions">
                            <a href="{{ route('user.datachanges.show', $datachange->id) }}"
