<x-user-layout title="">
    ...
    <div class="row">
        <div class="col">
            <h1>Add Workorder Shipment: {{ $workorder->W_WorkOrder }} - {{ $workorder->W_FirstName }}
                {{ $workorder->W_MiddleInit }} {{ $workorder->W_LastName }}</h1>
        </div>
        <div class="col-auto text-end">
            <a href="{{ route('user.shipments.index', ['workorder_id' => $workorder->W_WorkOrder]) }}"
    ...
    <div class="col-md-6">
        ...
        <form method="post"
              action="{{ route('user.shipments.store') }}">
            @csrf
