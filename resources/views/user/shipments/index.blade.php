<x-user-layout title="">
    ...
    <form method="get"
          ...
          id="searchform"
          action="{{ route('user.shipments.index') }}">
        @csrf
        <input type="hidden"
               name="search"
               value="1">
        ...
        <table class="table table-sm table-hover table-bordered w-auto">
            <thead>
                <tr>
                    ...
                    <th><a
                           href="{{ Request::fullUrlWithQuery(['sort_field' => 'service', 'sort_direction' => $sort_direction]) }}">Service</a>
                    </th>
                    <th><a
                           href="{{ Request::fullUrlWithQuery(['sort_field' => 'fee', 'sort_direction' => $sort_direction]) }}">Fee</a>
                    </th>
                    <th><a
                           href="{{ Request::fullUrlWithQuery(['sort_field' => 'tracking_number', 'sort_direction' => $sort_direction]) }}">Tracking
                            Number</a></th>
                    <th><a
                           href="{{ Request::fullUrlWithQuery(['sort_field' => 'ship_date', 'sort_direction' => $sort_direction]) }}">Ship
                            Date</a></th>
                    <th><a
            ...
            <tbody>
                @foreach ($shipments as $shipment)
                    <tr>
                        ...
                        <td><a href="{{ route('user.shipments.show', $shipment->id) }}"
    ...
    @if ($workorder_id)
        <a href="{{ route('user.shipments.create', ['workorder_id' => $workorder_id]) }}"
