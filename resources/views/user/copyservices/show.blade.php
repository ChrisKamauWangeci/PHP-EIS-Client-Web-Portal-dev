<x-user-layout title="">
    ...
    <table class="table table-hover table-bordered table-sm table-hover w-auto">
        ...
        @if ($subdomain == 'eisdev')
            <tr>
                ...
                <td><img src="/img/icon_{{ $copyservice->attestation_required }}.png" alt=""> {{ $copyservice->attestation_required ? 'Yes' : 'No '}}</td>
            </tr>
            <tr>
    ...
    @if ($subdomain == 'eisdev')
        ...
        <div class="col-5">
            ...
            <form method="post" enctype="multipart/form-data" action="{{ route('user.copyservices.fileupload', $copyservice->C_ID) }}">
                @csrf
                <input type="hidden" name="filename" value="{{ $copyservice->attestation_file }}">
    ...
    <a href="{{ route('user.copyservices.edit', $copyservice->C_ID) }}" class="btn btn-sm btn-secondary">Edit</a>
