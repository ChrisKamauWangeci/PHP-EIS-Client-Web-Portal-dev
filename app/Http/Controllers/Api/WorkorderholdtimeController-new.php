...
class WorkorderholdtimeController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if ($requirement) {
            ...
            $requirementText = collect(explode(',', $requirement))
                ->map(fn ($r) => Helper::requirementoption($r))
                ->implode(', ');

            $reasonText .= " - {$requirementText}";
        }
        ...
        if (in_array($this->subdomain(), ['eisdev', 'eisuat'])) {
            $workorderholdtime->statustrigger_id = $statustrigger->ID;
        }
    ...
    public function update(Request $request, Workorderholdtime $workorderholdtime)
    {
        ...
        $statusNoteEnd = $request->input('status_note_end');
        ...
        if ($workorderholdtime->requirement) {
            $requirementText = collect(explode(',', $workorderholdtime->requirement))
                ->map(fn ($r) => Helper::requirementoption($r))
                ->implode(', ');
        ...
        $statusTimeFormatted = now()->format('g:i:s A');

        $statustrigger = new Statustrigger();
        $statustrigger->WorkOrderNo = $workorder->W_WorkOrder;
        $statustrigger->statuscode = $statusCode;
        if ($requestor->R_Company == 'PLICO-WCL') {
            $statustrigger->laststatus = "{$statusCode}: ACTION REQUIRED END: {$reasonText} - {$statusNoteEnd} ({$statusTimeFormatted})";
        } else {
            $statustrigger->laststatus = "{$statusCode}: ACTION REQUIRED END: {$reasonText} - {$statusNoteEnd} ({session('user.contractor.C_Name')} {$statusTimeFormatted})";
        }
